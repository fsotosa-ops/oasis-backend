...
async def _get_superset_guest_token(user, user_org_id: str | None) -> str:
    ...
    try:
        ...
        logger.error("Cannot connect to Superset at %s: %s", SUPERSET_URL, exc)
        raise OasisException(
            code="superset_unreachable",
            message="No se pudo conectar con Superset. Verifica que el servidor esta accesible.",
            status_code=502,
        )
    except httpx.TimeoutException as exc:
        logger.error("Timeout connecting to Superset: %s", exc)
        raise OasisException(
            code="superset_timeout",
            message="Timeout al conectar con Superset.",
            status_code=504,
        )
    except KeyError as exc:
        logger.error("Unexpected Superset response format, missing key: %s", exc)
        raise OasisException(
            code="superset_bad_response",
            message="Respuesta inesperada de Superset.",
            status_code=502,
        )
    except httpx.HTTPError as exc:
        logger.error("HTTP error communicating with Superset: %s", exc)
        raise OasisException(
            code="superset_http_error",
            message="Error de comunicacion con Superset.",
            status_code=502,
        )
